      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 border-b border-border/80 pb-6">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-foreground mt-2.5 flex flex-wrap items-center gap-2">
            {config.miniSorteoTituloPrincipal ? (
              <span>{config.miniSorteoTituloPrincipal}</span>
            ) : (
              <>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1.5 max-w-2xl leading-relaxed">
            {config.miniSorteoSubtituloPrincipal || (
              <>
        ...
        <div className="relative overflow-hidden rounded-2xl border-2 border-amber-500/40 bg-gradient-to-br from-amber-500/10 via-card to-card p-6 shadow-md hover:border-amber-500/70 transition-colors">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="rounded-full bg-amber-500/20 border border-amber-500/40 px-3 py-1 text-[11px] font-black uppercase text-amber-300">
              {config.miniSorteoViernesBadge || `Todos los Viernes · ${formatearHora12(horaViernes)}`}
            </span>
          </div>
          ...
          <div className="mt-4">
            ...
            <h3 className="text-xl font-black text-foreground mt-0.5">
              {config.miniSorteoViernesTitulo || "Viernes de Tanque Lleno"}
            </h3>
            <p className="text-2xl font-black text-amber-400 mt-2">
              {config.miniSorteoViernesPremio || "₡50,000 en Gasolina Delta / Uno"}
            </p>
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
              {config.miniSorteoViernesDesc || "Llena el tanque de tu vehículo o motocicleta 100% gratis. También entregados formalmente si lo prefieres en efectivo."}
            </p>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-border/60 flex items-center gap-2 text-[11px] text-zinc-400">
            ...
            <span>{config.miniSorteoViernesAuditoria || "Auditado con la emisión oficial de los viernes de la JPS"}</span>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border-2 border-sky-500/40 bg-gradient-to-br from-sky-500/10 via-card to-card p-6 shadow-md hover:border-sky-500/70 transition-colors">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="rounded-full bg-sky-500/20 border border-sky-500/40 px-3 py-1 text-[11px] font-black uppercase text-sky-300">
              {config.miniSorteoDomingosBadge || `Todos los Domingos · ${formatearHora12(horaDomingos)}`}
            </span>
          </div>
          ...
          <div className="mt-4">
            ...
            <h3 className="text-xl font-black text-foreground mt-0.5">
              {config.miniSorteoDomingosTitulo || "Domingos de PlayStation 5 Extra"}
            </h3>
            <p className="text-2xl font-black text-sky-400 mt-2">
              {config.miniSorteoDomingosPremio || "Consola PS5 o ₡350,000 SINPE"}
            </p>
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
              {config.miniSorteoDomingosDesc || "Estrena una consola PlayStation 5 Slim Digital 0KM sellada de paquete o recibe ₡350,000 en efectivo por SINPE Móvil al instante."}
            </p>
          </div>

          <div className="mt-4 pt-3 border-t border-border/60 flex items-center gap-2 text-[11px] text-zinc-400">
            <Sparkles className="size-3.5 text-sky-400 shrink-0" />
            <span>{config.miniSorteoDomingosAuditoria || "Auditado directamente con la emisión dominical oficial de la JPS"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-1.5">
          <div className="flex items-center gap-2 font-bold text-sm text-foreground">
            <span className="text-base">🎟️</span> {config.miniSorteosGarantia1Titulo || "100% Automático"}
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed">
            {config.miniSorteosGarantia1Desc || (
              <>
        ...
        <div className="rounded-2xl border border-emerald-500/40 bg-emerald-950/20 p-4 space-y-1.5">
          <div className="flex items-center gap-2 font-bold text-sm text-emerald-400">
            <CheckCircle2 className="size-4 text-emerald-400 shrink-0" /> {config.miniSorteosGarantia2Titulo || "Sigues Jugando por el Carro"}
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed">
            {config.miniSorteosGarantia2Desc || (
              <>
        ...
        <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-1.5">
          <div className="flex items-center gap-2 font-bold text-sm text-foreground">
            <span className="text-base">📱</span> {config.miniSorteosGarantia3Titulo || "Depósito SINPE Inmediato"}
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed">
            {config.miniSorteosGarantia3Desc || "Los números ganadores se anuncian en vivo, se notifican directamente por WhatsApp y el premio se transfiere al instante."}
          </p>
        </div>
      </div>
